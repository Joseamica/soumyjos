...
Single-page static wedding invitation (index.html + `.ics` + preview images), deployed to Cloudflare Pages at `soumyjos.com`. Spanish (es_MX). The event is **20 de Marzo 2027** at **Cuadra San Cristóbal** (obra de Luis Barragán), **Atizapán de Zaragoza, Estado de México** (zona metropolitana de la CDMX) — the venue is in Los Clubes, Ciudad López Mateos, CP 52957. Earlier copy that said "Ciudad de México" was imprecise. Most guests open the link from inside WhatsApp on their phone.
...
- **Second page `invitacion.html`** (formal invitation, `/invitacion` rewrite) coexists with `index.html` (which stays untouched). Barragán/Cuadra San Cristóbal direction "D1 Planos Monumentales". Its own `:root` tokens (does NOT import `index.html`'s hard-coded hex). RSVP writes to the Firebase sibling node `rsvp/` (rules in `database.rules.json` — **must be published** in Firebase Console; the dashboard reads them via anonymous auth). Own calendar file `Soumi-Jos-Invitacion.ics` (real time, 1 VEVENT). Design spec + plan live in `docs/superpowers/`.
- **Conscious canon exception (countdown):** the canon lists "countdown clocks" as an anti-reference, but the couple asked for one on `invitacion.html`. It's resolved as a quiet editorial line **over cream** (not a wall, no flip, days only) — not an animated clock. It lives only on the formal invitation; the Save the Date keeps its own `updateCountdown()`.
